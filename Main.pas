...
type
  ...
  public
    { Public declarations }
    API_Key, City, country, clouds, windspeed, rain, winddirection, windgust,
      Humidity, Temperature, Roofstatus: String;
    UpdateSeconds: Integer; // Tid mellan uppdatering av väderfil
    SaveWeather: Boolean;
    OK: Boolean;
    RoofFileName, WeatherFilename, Longitude, Latitude, MaxWindSpeed: String;
...
begin
  ...
  try
    ...
    begin
      ...
      WriteLogFile('Error från Openweather API: ' + Response.StatusCode.ToString
...
begin
  ...
  begin
    // Hit har den ännu aldrig kommit 20230908 NS
    Memo1.Lines.Clear;
...
begin
  ...
  try
    ...
    begin // Om filen finns, testa om den går att läsa.
    ...
    // Om den inte gått att läsa tre ggr skriv till log och sätt
    // taket som stängt.
    if RoofFileReadErrors > 3 then
    begin
      WriteLogFile('Filen: ' + FN + ' kan inte öppnas efter tre försök.', 1);
      Roofopen := False
    end
  ...
  try
    ...
    begin
      ...
      S := 'Datum:tid' + Separator + 'Hastighet (m/S)' + Separator +
        'Byar (m/s)' + Separator + 'Vindriktning (grader)' + Separator + 'OK?';
      WriteLn(F, S);
      CloseFile(F);
...
begin
  ...
  try
    ...
    if (Roofstatus = ' öppet') and
      (StrToFloat(windspeed, FSe) < StrToFloat(MaxWindSpeed, FSe)) then
      OK := true
    else
      OK := False;
